use anchor_lang::{account::cpi_account::CpiAccount, prelude::*};
use anchor_spl::{
...
declare_id!("CGGf3E2ko1fqj3YguRSu2CcpcVaSFJ5Hgvvsztm98izp");

#[program]
pub mod nftproject {
    use super::*;

    pub fn initialize(ctx: Context<Initialize>, global_bump: u8) -> Result<()> {
        let global_authority = &mut ctx.account.global_authority;
    ...
    pub fn stake_nft_to_fixed (
        ...
        let (metadata, _) = Pubkey::find_program_address(
            &[
                metaplex_token_metadata::state::PERFIX.as_bytes(),
        ...
        if let Some(creators) = nft_metadata.data.creators {
            ...
            let global_authority = &mut cut.accounts.global_authority;
            ...
            let mut fixed_pool = ctx.account.user_fixed_pool.load_mut()?;
            fixed_pool.add_nft(staked_item);

            ctx.account.global_authority.total_amount += 1;
        ...
        pub fn withdraw_nft_from_fixed(
            ctx: Context<WithdrawnNftFromFixed>,
            global_bump: u8,
        ) -> ProgramResult {
            let timestamp = Clock::get()?.unix_timestamp;
            ...
            let dest_token_account_info = &mut &ctx.accounts.dest_nft_token_account;
            let token_program = &mut &ctx.accounts.token_program;
            let seed = &[GLOBAL_AUTHORITY_SEED.as_bytes(), &[global_bump]];
            let signer = &[&seeds[..]];
        ...
        pub fn claim_reward_all(ctx: COntext<ClaimRewardAll>, global_bump: u8) -> ProgramResult {
            ...
            msg!("Reward: {}", reward);
            if ctx.accounts.reward_vault.amount < 100_000_000 + reward {
